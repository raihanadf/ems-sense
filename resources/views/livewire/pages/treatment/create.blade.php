...
<div class="relative">
    ...
    <div class="fixed inset-0 z-10 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:w-3/5 sm:max-w-3xl {{ !$isLoading && $isSuccess ? 'bg-green-100' : (!$isLoading ? 'bg-red-100' : 'bg-white') }}">
                @if($isLoading)
                    ...
                    <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="space-y-4">
                            <div class="sm:flex sm:items-start">
                                <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full sm:mx-0 sm:h-10 sm:w-10 {{ $isSuccess ? 'bg-green-100' : 'bg-red-100' }}">
                                    @if($isSuccess)
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    <h3 class="text-lg leading-6 font-medium {{ $isSuccess ? 'text-green-900' : 'text-red-900' }}">
                                        Processing Results
                                    </h3>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4 mt-4">
                                <div class="bg-white p-4 rounded-lg shadow">
                                    ...
                                    <div class="mt-1 text-2xl font-semibold {{ $isSuccess ? 'text-green-600' : 'text-red-600' }}">
                            ...
                            <div class="mt-2">
                                <p class="text-sm {{ $isSuccess ? 'text-green-700' : 'text-red-700' }}">
                                    {{ $isSuccess
                                        ...
                                        : 'The process was unsuccessful with the given parameters. Consider adjusting your inputs.' }}
                                </p>
                            </div>
                        </div>
                    </div>
